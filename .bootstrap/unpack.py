...
parts = sorted(bootstrap.glob("payload-*.txt"))

if not parts:
    raise RuntimeError("Rune bootstrap payload is missing")
...
with tarfile.open(archive, "r:gz") as bundle:
    for member in bundle.getmembers():
        if member.issym() or member.islnk():
            raise RuntimeError(f"links are not allowed in bootstrap payload: {member.name}")
