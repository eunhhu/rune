...
import { InputSource, Key, MouseButton } from "@rune/sdk";
import {
  InputDevice,
  InputEdge,
  Opcode,
    ...
    for (const statement of this.sourceFile.statements) {
      if (ts.isVariableStatement(statement)) {
        ...
        for (const declaration of statement.declarationList.declarations) {
          ...
          // Rune modules may contain arbitrary control-plane TypeScript next to realtime
...
export function compileSource(source: string, options: CompileOptions = {}): CompileResult {
  const compiler = new Compiler(source, {
    fileName: options.fileName ?? "macro.rune.ts",
