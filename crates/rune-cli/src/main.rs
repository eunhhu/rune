...
fn run() -> Result<(), String> {
    ...
    match command.as_str() {
        ...
        "inspect" => {
            let path = args.next().ok_or_else(|| "inspect requires a .rune.bin path".to_owned())?;
...
fn format_trigger(trigger: Trigger) -> String {
    let event = InputEvent {
        ...
        source: match trigger.source {
            rune_core::SourceFilter::Synthetic => InputSource::Synthetic,
            rune_core::SourceFilter::Physical | rune_core::SourceFilter::Any => {
                InputSource::Physical
            }
        },
    };
    let source = match trigger.source {
        rune_core::SourceFilter::Physical => "physical",
        rune_core::SourceFilter::Synthetic => "synthetic",
        rune_core::SourceFilter::Any => "any",
    };
...
fn print_usage() {
    eprintln!(
        "Rune native VM utility\n\n\
         Usage:\n\
           rune-sim inspect <program.rune.bin>\n\
           rune-sim simulate <program.rune.bin> <event>...\n\n\
         Event syntax:\n\
           key-down:Q\n\
           key-up:LeftShift\n\
           mouse-down:left\n\
...
mod tests {
    ...
    use rune_core::{key, Edge, InputDevice, InputSource, MouseButton};
