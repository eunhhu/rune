...
pub unsafe extern "C" fn spellwire_engine_free(engine: *mut SpellwireEngine) {
    if !engine.is_null() {
        // SAFETY: A non-null pointer must have been returned by `spellwire_engine_new`
...
pub unsafe extern "C" fn spellwire_engine_dispatch(
    ...
    // SAFETY: The pointer is checked and only dereferenced for this call. SpellwireEngine
