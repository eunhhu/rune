...
impl fmt::Display for DecodeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Truncated => f.write_str("truncated Rune bytecode"),
            Self::InvalidMagic => f.write_str("invalid Rune bytecode magic"),
            Self::UnsupportedVersion(version) => {
                write!(f, "unsupported Rune bytecode version {version}")
            }
            ...
            Self::SizeOverflow => f.write_str("Rune bytecode size overflow"),
            Self::TrailingBytes(count) => write!(f, "Rune bytecode has {count} trailing bytes"),
        }
    }
}
